  ...
  return (
    <main className="setup-view animate-fade-in">
      ...
      <div className="setup-card" style={{ padding: '1rem' }}>
        {/* 원형 프로그레스 - 레이아웃 고정을 위해 항상 렌더링 */}
        <div style={{ marginBottom: '0.6rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '1rem', minHeight: '110px' }}>
          {/* 왼쪽 범례 - 총 에너지 */}
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.3rem', width: '60px' }}>
          ...
          <div style={{ position: 'relative', width: '110px', height: '110px', flexShrink: 0 }}>
            <svg width="110" height="110" style={{ transform: 'rotate(-90deg)' }}>
              ...
              <circle cx="55" cy="55" r="48" fill="none" stroke="var(--primary)" strokeWidth="6" strokeDasharray={`${2 * Math.PI * 48}`} strokeDashoffset={totalWorkTime > 0 ? 0 : `${2 * Math.PI * 48}`} strokeLinecap="round" />
              <circle cx="55" cy="55" r="38" fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth="6" />
              <circle cx="55" cy="55" r="38" fill="none" stroke={focusTime > totalWorkTime ? 'var(--error)' : '#FFD700'} strokeWidth="6" strokeDasharray={`${2 * Math.PI * 38}`} strokeDashoffset={`${2 * Math.PI * 38 * (1 - Math.min(totalWorkTime > 0 ? focusTime / totalWorkTime : 0, 1))}`} strokeLinecap="round" style={{ transition: 'stroke-dashoffset 0.3s ease' }} />
            </svg>
          </div>

          {/* 오른쪽 범례 - 집중 비율 */}
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '0.3rem', width: '60px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#FFD700' }}></div>
              <span style={{ fontSize: '0.7rem', opacity: 0.7 }}>집중 비율</span>
            </div>
            <div style={{ fontSize: '1.1rem', fontWeight: 900, color: (totalWorkTime > 0 && focusTime > totalWorkTime) ? 'var(--error)' : '#FFD700' }}>
              {totalWorkTime > 0 ? Math.round((focusTime / totalWorkTime) * 100) : 0}%
            </div>
          </div>
        </div>
